...
- **Freeze-frame preview showing black in Program Monitor**: Freeze clips now force decoder seeks to use accurate, non-keyframe-seeking flags in the live preview path. This prevents single-frame freeze seek windows from dropping the held frame under `KEY_UNIT` seeking and fixes black freeze holds while preserving export behavior.
