...
- **App freeze on timeline edit during playback**: Fixed a second deadlock in `teardown_slots()` where setting decoders to Null *before* disconnecting them from the compositor caused the main thread to block on pad locks held by the compositor's streaming thread. Reordered teardown to: (1) release compositor/audiomixer request pads (disconnects data flow), (2) remove elements from pipeline (unlinks pads), (3) set to Null (now safe because elements are disconnected).
