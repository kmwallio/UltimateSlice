...
- **Auto-linked same-source A/V placement**: Drag/drop and MCP clip placement now auto-create linked video+audio clip pairs for eligible sources with both video and audio streams, target matching video/audio tracks together, and suppress duplicate embedded video-track audio while the linked audio-track peer exists. Source monitor operations (Append, Insert, Overwrite) use single-clip placement for reliability.
