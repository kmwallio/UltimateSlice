...
- **Incremental boundary paths re-enabled ("no-touch-retained" strategy)**: Root-caused the static-frame regression to `compositor.seek_simple(FLUSH, ZERO)` propagating upstream through ALL sink pads to retained decoders, causing a double-flush-seek that corrupted their streaming state. Rewrote both incremental paths: remove-only now tears down exiting slots without pausing or touching retained decoders; add-only pauses, builds + seeks only new decoders while retained decoders keep their paused preroll buffers undisturbed. Estimated ~90% faster for remove-only (~100ms vs 1.3s) and ~50% faster for add-only (~600ms vs 1.3s) boundary transitions.
