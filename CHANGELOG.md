...
### Added
- **Auto-linked same-source A/V placement**: Append, Insert, Overwrite, drag/drop, and MCP clip placement now auto-create linked video+audio clip pairs for eligible sources with both video and audio streams, target matching video/audio tracks together, and suppress duplicate embedded video-track audio while the linked audio-track peer exists.
...
### Fixed
- **Volume slider storing dB instead of linear**: The Inspector volume slider handler was saving the raw dB slider value into `clip.volume` (which expects a linear multiplier). On next inspector refresh, the negative dB value was converted back via `linear_to_db_volume()`, mapping to -100 dB (mute). Now converts to linear before storing.
