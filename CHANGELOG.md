...
- **Extended color grading fields**: Added 8 new clip fields — `exposure`, `black_point`, `highlights_warmth`, `highlights_tint`, `midtones_warmth`, `midtones_tint`, `shadows_warmth`, `shadows_tint` — with Inspector sliders and MCP `set_clip_color` support. Exposure applied as gamma correction in preview (videobalance brightness approximation) and export (FFmpeg `eq` gamma). Black point and per-tone warmth/tint applied via frei0r `3-point-color-balance` (per-channel RGB offsets) in preview and FFmpeg `colorbalance` (per-range per-channel shifts) in export.
