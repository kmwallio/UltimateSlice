...
### Added
- **Keyframe dopesheet editor (phase 1)**: Added a new **Keyframes** panel in the right sidebar with lane visibility toggles and a timeline-based dopesheet for selected clips. You can select keyframes, drag them to retime, add keyframes at the playhead, remove selected keyframes, and apply interpolation modes (Linear/Ease In/Ease Out/Ease In/Out). Keyframe edits are committed through undo/redo command history, and the panel stays synchronized with clip selection and playhead movement.
