...
- **Program monitor playhead freeze with 3+ video tracks**: Fixed the playhead freezing when a higher-priority overlay clip becomes active. During mid-playback clip switches the playbin was transitioned directly to Playing before seeking, so the seek was silently rejected and the pipeline played from source position 0. A stale `query_position` value from the previous clip then caused the timeline position to jump forward, and the backward clamp locked the playhead at the jumped value permanently. Restructured `load_clip_idx` to always preroll through PAUSED first, seek in PAUSED (guaranteed to succeed), wait for the seek to settle, anchor to the actual post-seek position, and only then resume Playing.
