...
### Fixed
- **Source preview freeze on high-resolution HEVC media**: Importing a 5.3K GoPro HEVC file (5312×2988, H.265, ~60 Mbps) and playing it in the source preview could freeze the application. The VA-API decoder outputs GPU memory (`video/x-raw(memory:VAMemory)`) at full resolution, requiring an expensive GPU→CPU download per frame before the CPU prescale could downscale to 1080p. Four fixes: (1) added a leaky queue (`max-size-buffers=2, leaky=downstream`) after the prescale capsfilter to decouple the decode/prescale thread from the effects/display chain, preventing pipeline backpressure from blocking the UI; (2) source preview now checks for and uses proxy files when available, and auto-requests proxy transcodes for video media loaded into the source monitor — when a proxy completes, the source player automatically reloads with the lighter proxy file; (3) added GStreamer bus error monitoring on the source preview pipeline so decoder/negotiation errors are logged instead of silently freezing; (4) fixed STATE_LOCK deadlock in `Player::play()`/`pause()`/`stop()` where `set_state()` could block indefinitely if the pipeline had a pending async state change (e.g. gtk4paintablesink GL preroll) — state transitions now defer via a repeating timeout when async work is in progress, keeping the main thread responsive.
