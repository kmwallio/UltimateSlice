...
### Fixed
- **Program monitor preview stops with 3+ video tracks**: Fixed the preview halting when the highest-priority overlay clip ends instead of continuing playback on lower tracks. The playbin seek issued during a mid-playback clip switch was silently rejected because the pipeline hadn't reached PAUSED state yet, causing the new clip to play from position 0 instead of the correct source offset. Added a preroll wait in the Playing-state path of `load_clip_idx` so the seek is accepted, and added a timeline-position safety check to the end-of-clip logic so clip transitions fire even if the source position is out of sync.
