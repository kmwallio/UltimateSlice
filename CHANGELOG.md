...
### Changed
- **Preview-resolution compositing**: The entire preview pipeline (per-slot effects, compositor, output) now runs at the auto-selected preview resolution instead of always compositing at full project resolution and then downscaling. With Half quality (divisor 2), this reduces per-frame pixel work by ~4×; Quarter (divisor 4) reduces it by ~16×. Crop values, zoom, and position are scaled to match the render resolution automatically.
- **Default preview quality changed to Auto**: Preview quality now defaults to `Auto` instead of `Full`, dynamically selecting Half or Quarter resolution based on the program monitor canvas size. This provides smoother playback out of the box while still allowing users to switch to Full quality when needed.
...
### Fixed
- **Preview FPS regression (0–5 FPS after leaky queue fix)**: After making the display queue leaky to fix 3-track deadlock, preview frame rate dropped to 0–5 FPS because the pipeline was non-live — the compositor ran uncapped at 1000+ FPS and the leaky queue dropped 99%+ of frames. Fixed by setting `is-live=true` on both background sources (black `videotestsrc` for compositor, silent `audiotestsrc` for audiomixer), which forces both aggregators into live mode where they pace output at the negotiated framerate using the system clock.
- **Audio-less clips stalling audiomixer**: Clips without an audio stream (e.g. screencasts, image sequences) still had an audiomixer sink pad requested, leaving an unfed pad that could stall the aggregator in non-live mode. Added `probe_has_audio_stream()` using GStreamer Discoverer to detect audio-less clips, and the slot builder now skips the `audioconvert → audiomixer` path entirely for such clips.
- **3-track playback deadlock (0 FPS with 3+ video tracks)**: With three or more overlapping video tracks, playback stalled at 0 FPS after an initial burst of ~12 frames. The root cause was backpressure from `gtk4paintablesink` (which renders on the GTK main thread) propagating through the non-leaky display queue (`q1`) back through the `tee`, blocking the compositor, which in turn blocked all decoder slot queues — a full pipeline deadlock. Fixed by making the display queue leaky (`leaky=downstream, max-size-buffers=2`) so it drops old frames instead of blocking. Per-slot decoder→compositor queues remain non-leaky to preserve frame accuracy on 1–2 track projects. Result: 3-track playback went from 0 FPS to 91–95 FPS.
