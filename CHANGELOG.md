...
### Fixed
- **FCPXML transform keyframe FCP compatibility**: Writer now matches Final Cut Pro's native format for `<adjust-transform>` keyframe params — uses lowercase `<param name="scale">` (was `Scale`), omits the `value` attribute from `<param>` elements that contain `<keyframeAnimation>` children, and omits inline `position`/`scale`/`rotation` attributes from the `<adjust-transform>` element for properties that have keyframes. This fixes FCP ignoring keyframed transforms when importing UltimateSlice-exported `.fcpxml` files.
