...
### Fixed
- **FCPXML transform keyframe FCP compatibility**: Writer now matches Final Cut Pro's native format for `<adjust-transform>` keyframe params — uses lowercase `<param name="scale">` (was `Scale`), omits the `value` attribute from `<param>` elements that contain `<keyframeAnimation>` children, omits inline `position`/`scale`/`rotation` attributes from the `<adjust-transform>` element for properties that have keyframes, and omits `interp` attribute from transform `<keyframe>` elements (FCP rejects transform keyframes with interpolation attributes). This fixes FCP ignoring keyframed transforms when importing UltimateSlice-exported `.fcpxml` files.
- **FCPXML keyframe time domain correction**: Parser now converts all native FCPXML keyframe times (position, scale, rotation, opacity, volume, pan) from absolute source-media time to clip-local time by subtracting the clip's raw `start` attribute value (`raw_source_start_ns`). Writer adds it back when exporting. Previously, transform and opacity keyframes were stored with absolute times (~72887s for GoPro media), causing them to display at wrong positions and evaluate incorrectly when imported from pure FCP files. Volume and pan keyframes previously used `source_in` (model-relative) rather than the raw FCPXML `start` value, which failed when the asset had a non-zero start attribute.
