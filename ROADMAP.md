...
- [x] Audio Track Mixer (Panel for track-level volume, panning, effects, and bus routing) — Phase 1: per-track gain faders (−∞ to +12 dB), stereo pan, VU meters, mute/solo, master strip. Bottom panel alongside Keyframes/Transcript/Markers. Track gain/pan compose with per-clip volume keyframes and ducking in both GStreamer preview and FFmpeg export. FCPXML and OTIO persistence. Three new MCP tools: `set_track_gain`, `set_track_pan`, `get_mixer_state`. Phase 2 (bus routing, per-track effects) deferred.
