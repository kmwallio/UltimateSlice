...
- [ ] **Context-aware clip search** — a natural-language search bar at the top of the Media Library panel that filters the visible library list by **what's actually in the clip**, not just filename/metadata. Builds on three independent signals so the matcher can combine them:
  - **Transcript / spoken content**: full-text and fuzzy search across the per-clip STT word timings already produced by the existing Speech-to-Text pipeline (leveraging `SttCache` + the subtitle word lists surfaced by the Transcript Editor). Queries like `"find the sticker"` or `"everyone's here"` resolve to the clips whose dialogue contains those tokens, ranked by match density. Zero new models — this is essentially free once the index is built, since the data is already there.
  ...
  - **UI**: a single search field at the top of the media-browser panel that returns unified results ranked across all three signals. Transcript hits highlight the matched word in a tooltip; visual hits show the best-match keyframe thumbnail; tag hits surface the matched tag chip. Smart Collections can save any search as a persistent filter. No separate global palette for now — keep the surface area tight and lean on the existing browser filter affordances. If the query field is empty, the browser behaves exactly like today.
  - **Scope notes**: the embedding model is the new dependency — everything else builds on existing infrastructure. Ship transcript search first (it's almost free), then visual search, then auto-tagging as the final step. Each phase is independently useful so shipping them incrementally gives real value along the way.
