...
- [x] Render-and-Replace (Bake complex effect stacks into temporary high-quality clips)
  ...
  - [x] Phase 1c: export pipeline gating so exports also read from the sidecar when ready (superseded by the completed Phase 1c export-gating entry below).
  ...
  - [x] Phase 3: expand baked scope to chroma key, HSL qualifier, shape masks, vidstab, and audio effects on single clips; multi-clip batch action. The leaf-clip signature (`fold_baked_fields`), bake filter (`run_leaf_bake`), and both neutralize sites now cover chroma key (ProRes 4444 alpha), HSL qualifier, vidstab (two-pass detect), LADSPA audio (`-af`), and **static, untracked rectangle/ellipse/path shape masks** — rect/ellipse via an inline `geq` alpha written against ffmpeg's `W`/`H` geq vars (resolution-independent, since the leaf bake runs at the source's native res in clip-local space), path masks via a rasterized PGM brought in with `movie`+`scale2ref`+`alphamerge`. The earlier preview-side neutralization only zeroed the Phase-1b fields, so HSL/LADSPA/chroma **double-applied in the Program Monitor** until this phase brought `neutralize_baked_effects_for_sidecar_clips` into lock-step with the export-side `neutralize_baked_fields`. **Kept live (carve-outs):** tracked or keyframed masks (per-frame geometry can't be a single static bake) and path-mask + chroma-key combos (the path `alphamerge` would clobber the keyed alpha — chroma still bakes, masks stay live). Multi-clip batch action shipped via `TimelineState::set_render_replace_for_all_selected` + the Inspector "Apply to selected" button (visible only when >1 bakeable clip is selected). 4 new tests.
