...
///
/// Returns `(widget, program_player)` — the caller must call
/// `program_player.borrow_mut().load_clips(...)` whenever the project changes,
/// and poll position in the main timer.
pub fn build_program_monitor(
    ...
    // 100ms timer: poll position + update timecode label
    {
        let pp = program_player.clone();
