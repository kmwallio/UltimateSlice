...
- In the FFmpeg `coloradj_RGB` bridge path, export now applies conservative tint-delta attenuation and a small cool-side temperature attenuation versus preview parameters to reduce cross-runtime drift while keeping neutral/warm behavior stable. Tonal (shadows/midtones/highlights) residuals remain under active calibration.
