...
- In the FFmpeg `coloradj_RGB` bridge path, export now applies a conservative tint-delta attenuation versus preview parameters to reduce cross-runtime tint drift (especially strong green/magenta endpoints). Temperature and tonal (shadows/midtones/highlights) residuals remain under active calibration.
