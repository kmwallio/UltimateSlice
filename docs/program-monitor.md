...
permalink: /docs/program-monitor/
---
...
When a clip has a speed multiplier set (see [Inspector](inspector)), the program monitor plays it at that rate using GStreamer's rate-seek mechanism. Audio pitch is **not** corrected in the preview (it sounds higher/lower pitched). The exported file uses `atempo` for proper pitch correction.
